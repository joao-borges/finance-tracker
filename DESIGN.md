...
- **Unknown `simplefin_id` but content hash collides with a live row:** a re-import under a **rotated id** — a bridge reconnect regenerates every account and transaction id. → **quarantine** (`is_dedup = true`, restorable), saved *with* the new id so the next sync skips it at tier 1. Each old transaction quarantines exactly once. Hashes are snapshotted before the run, so same-day identical legitimate charges within one payload all survive.
...
- **Statement hash (both paths):** a second key over the RAW statement text (`ContentHashing.ofStatement` — lowercase, alphanumerics only) computed from the bridge `description` on SimpleFIN rows and from the CSV description on CSV rows. It bridges the gap the merchant hash cannot: SimpleFIN's payee is a derived pretty name ("Starbucks") while a CSV carries the raw descriptor ("STARBUCKS 8007827282 800-782-7282"). Either key colliding with a live row → quarantine. This is what lets one account ingest from **CSV and SimpleFIN simultaneously** without duplication (e.g. filling a stalled bridge feed from a bank export).
- **Cross-source date probing:** banks skew dates across sources — a CSV export carries the *transaction* date while the bridge reports the (1–2 days later) *posted* date — so statement hashes from OTHER sources are probed at ±2 days, while same-source comparison stays exact-day (keeping same-day identical legit charges safe within a source).
- **Hashes are keyed on the account ID, not the name.** Names get renamed and accounts get merged; a name-keyed hash silently stops matching afterwards (this produced real duplicates twice). A merge re-keys the moved rows under the canonical account, and `TransactionHashMaintenance` rebuilds any stale hashes at boot (idempotent, no-op when consistent).
...
- **SimpleFIN health check** — on demand only, from the "Health check" button on the Imports page (`POST /api/simplefin/health-check`): the last sync's outcome plus any linked account whose bridge balance feed is >48h stale. Green when everything is fresh, yellow otherwise. **There is deliberately no scheduled digest.** A daily 10:00 version existed and was removed: the bridge reports a connection as healthy while its auth is already dead — the failure only surfaces when an import actually shuffles the connections — so a daily green message was noise that also taught us to ignore the yellow ones.
